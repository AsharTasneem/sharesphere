...
export const supabaseRequestsService = {
     ...
     * Create new request
     */
    create: async (requestData: Omit<Request, 'id' | 'createdAt' | 'updatedAt'>): Promise<Request> => {
     ...
     */
    update: async (id: string, requestData: Partial<Request>): Promise<Request> => {
        ...
        if (requestData.pickupCode !== undefined) update.pickup_code = requestData.pickupCode;
        if (requestData.returnCode !== undefined) update.return_code = requestData.returnCode;
        if (requestData.pickupDate) update.pickup_date = toDbDate(requestData.pickupDate);
        if (requestData.returnDate) update.return_date = toDbDate(requestData.returnDate);
        if (requestData.ownerNotes !== undefined) update.owner_notes = requestData.ownerNotes;
        if (requestData.cancellationReason !== undefined) update.cancellation_reason = requestData.cancellationReason;
        if (requestData.depositRefunded !== undefined) update.deposit_refunded = requestData.depositRefunded;
        if (requestData.refundAmount !== undefined) update.refund_amount = requestData.refundAmount;
        ...
        if (requestData.paymentId !== undefined) update.payment_id = requestData.paymentId;
