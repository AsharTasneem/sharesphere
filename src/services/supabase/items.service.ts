...
export const supabaseItemsService = {
   ...
   */
  create: async (
    itemData: Omit<Item, "id" | "createdAt" | "updatedAt" | "metadata">
  ): Promise<Item> => {
    const insert: TablesInsert<"items"> = {
      ...
      blocked_dates: itemData.availability.blockedDates?.map((d) =>
        toDbDate(d)
      ),
      ...
      published_at:
        itemData.status === "published" ? new Date().toISOString() : undefined,
    };
  ...
  update: async (id: string, itemData: Partial<Item>): Promise<Item> => {
    ...
    if (itemData.pricePerDay !== undefined)
      update.price_per_day = itemData.pricePerDay;
    ...
    if (itemData.primaryImage !== undefined)
      update.primary_image = itemData.primaryImage;
    if (itemData.pickupInstructions !== undefined)
      update.pickup_instructions = itemData.pickupInstructions;
    if (itemData.pickupWindow !== undefined)
      update.pickup_window = itemData.pickupWindow;
    ...
    if (itemData.location) {
      if (itemData.location.address !== undefined)
        update.address = itemData.location.address;
      if (itemData.location.city !== undefined)
        update.city = itemData.location.city;
      if (itemData.location.state !== undefined)
        update.state = itemData.location.state;
      if (itemData.location.displayAddress !== undefined)
        update.display_address = itemData.location.displayAddress;
    ...
    if (itemData.availability) {
      if (itemData.availability.type)
        update.availability_type = itemData.availability.type;
      if (itemData.availability.blockedDates !== undefined) {
        update.blocked_dates = itemData.availability.blockedDates?.map((d) =>
          toDbDate(d)
        );
      }
